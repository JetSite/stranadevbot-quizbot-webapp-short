...
const Item = ({ item, navigate, isFetching }) => {
  return <div onClick={() => { !isFetching && navigate('/collectionItem', { state: { idItem: item.id } }) }} className='p-5 w-full bg-itemBg border border-borderItem'>
    <div className='mb-2 flex justify-center'>
      ...
      /> */}
      <img crossOrigin="anonymous" className='object-contain min-h-32' src={item?.attributes?.image[0]?.link} />
    </div>
    <div className='mb-2'>
      <p className='text-xs mb-1'>{item?.attributes?.area?.value ? <p className="text-center">
        {item?.attributes?.property_type}{' '}
        {
          item?.attributes?.area?.value
        }
        м²
      </p> : item?.attributes?.property_type}</p>
      <div className='mb-1 border-t border-borderItem' />
      <p className='text-[10px]'>{item?.attributes?.price?.value.toLocaleString('ru') || 0} ₽</p>
    </div>
